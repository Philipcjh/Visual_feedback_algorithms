...
int main(int argc, char* argv[])
{
    const char* keys =
        ...
        "{ l left     | 1-1.bmp          | specify left image  }"   //彩虹糖是1-1.bmp,1-2.bmp
        "{ r right    | 1-2.bmp | specify right image }"            //杯子是2-1.bmp,2-2.bmp
    ...
    int match_ele = 20;
    std::nth_element(matches.begin(),matches.begin() + match_ele-1,matches.end());
    ...
    Mat Hbg,Hbm;
    Mat_<double> InitialPose = (cv::Mat_<double>(1, 6) << 534.88, -73.81, 463.46, 2.6936, 2.0509, -0.2141);
    ...
    Mat Rbm, Tbm;
    RT2R_T(Hbm, Rbm, Tbm);
    Mat_<double> T_com = (cv::Mat_<double>(3, 1) << 0, 0, 245);//固定补偿（软体手长度）
    Tbm += T_com;
